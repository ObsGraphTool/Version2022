...
/*for(TRANSITIONS::const_iterator j=transitions.begin();!(j==transitions.end());j++)
    ...
    if(strlen(Int_trans)>0 ||strlen(Formula_trans)>0)
      {      
	...
	if(!strcmp(f, "ALL"))
	  { 
	    ...
	    for(TRANSITIONS::const_iterator j=transitions.begin();!(j==transitions.end());j++)
	      {
		//cout<<"test ici : "<<j->name<<"#"<<endl;
		//cout<<"test ici : "<<pos_trans(transitions,j->name)<<"#"<<endl;
		Formula_Trans.insert(pos_trans(transitions,j->name));
	      }
	  }
	else 
	  { if(strcmp(f, "EMPTY"))
